...
export async function GET(
  ...
  try {
    if (!checkAdminSecret(request)) return errorResponse(getErrorMessage(ERROR_CODES.UNAUTHORIZED), requestId, 401)
    ...
    const [sessionCount, costByFeature, recentLogs, recentSessions] = await Promise.all([
      prisma.chatSession.count({ where: { userId: id, deletedAt: null } }),
      prisma.lLMLog.groupBy({
        by: ['feature'], where: { userId: id },
      ...
      prisma.chatSession.findMany({
        where: { userId: id, deletedAt: null }, orderBy: { createdAt: 'desc' }, take: 5,
        select: { id: true, title: true, createdAt: true, totalTurns: true, averageScore: true },
      }),
    ])
...
export async function PATCH(
  ...
  try {
    ...
    if ('postesRecherches' in body) {
      ...
      data.postesRecherches = body.postesRecherches
    }
    if ('resetCvRateLimit' in body && body.resetCvRateLimit) {
      data.cvAnalysisCount = 0
      data.cvAnalysisAt = null
    }
    if ('resetChatReportRateLimit' in body && body.resetChatReportRateLimit) {
      data.chatReportCount = 0
    }
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.$transaction([
      prisma.user.update({
        where: { id },
        data: { email: `deleted-${id}@deleted.invalid`, profile: Prisma.DbNull, deletedAt: now },
      }),
